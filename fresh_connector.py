...
def main():
    parser = argparse.ArgumentParser(description="Create and close tickets in freshservice.",
                                     prog="python fresh_connector.py", epilog="Author: Andrew Jaffie")
    parser.add_argument("-s", "--server", required=True, type=str, help="the icinga object to report as down or up", dest="server")
    parser.add_argument("--no-ticket", dest="noticket", action="store_true", default=False, help="don't create a ticket, only write to db")
    parser.add_argument("-p", "--priority", dest="priority", type=int, help="priority of the ticket", choices=[i for i in range(1,5)], default=2)
...
def build_payload(args, config):
    ...
    if not args["description"] == "":
        ...
        payload["description"] = 'The server "'+args["server"]+'" is now offline. Please investigate and resolve the issue.'
